...
**Required Fields (All 11)** - Conceptual fields (map to YAML when embedding):
...
- AI Log: <relative path to AI log file>
- Source: <source identifier>

Notes:

- In YAML front matter, use `ai_generated: true` (boolean) instead of the string "Yes".
- Chat ID maps to `chat_id` field in YAML.
- AI Log maps to `ai_log` field in YAML.
- Source maps to `source` field in YAML.
...
interface CopilotChat {
  ...
  model: string; // Underlying AI model (e.g., "openai/gpt-4o@2024-11-20")
